...
namespace Portfolio.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadPhoto([FromForm] UploadPhotoRequestDto requestDto)
        {
            try
            {
                var updated = await _profileService.UploadProfilePhotoAsync(_currentUser.UserId!.Value, requestDto.File);
                return Ok(updated);
            }
        ...
        [Authorize]
        [HttpPost("me/resume")]
        public async Task<IActionResult> UploadOrReplaceResume([FromForm] UploadResumeRequestDto requestDto)
        {
            try
            {
                var updated = await _profileService.UploadOrReplaceResumeAsync(_currentUser.UserId!.Value, requestDto.File);
                return Ok(updated);
            }
            catch (ArgumentException ex)
            {
