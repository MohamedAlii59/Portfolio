...
namespace Portfolio.Controllers
{
    ...
    public class ProjectsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadImages(int id, [FromForm] UploadProjectImagesRequestDto request)
        {
            try
            {
                var images = await _projectService.UploadImagesAsync(_currentUser.UserId!.Value, id, request.Files);
                return Ok(images);
            }
            catch (KeyNotFoundException)
            {
